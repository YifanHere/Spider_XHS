...
### 🧹后处理：过滤无人声解说视频
> 说明：下载的媒体默认保存在 `datas/media_datas/昵称_userid/标题_noteid/`，视频文件为 `video.mp4`，元数据为 `info.json`。
...
2. 安装 ffmpeg（确保命令行可用 `ffmpeg`）。

3. 运行后处理脚本（默认只做标记，不删除）：
```
python postprocess_audio.py --action mark
```

4. 如需删除无人声视频（保留 `info.json` 记录）：
```
python postprocess_audio.py --action delete
```

常用参数：
- `--media-dir` 指定媒体目录（默认 `datas/media_datas`）
- `--min-speech-seconds` 最短语音秒数阈值（默认 1.0）
- `--min-speech-ratio` 语音占比阈值（默认 0.02）
- `--threshold-mode` 阈值判定方式（`any`/`all`）
- `--force` 强制重新检测已处理视频

处理结果写入每个视频目录下的 `info.json`，新增字段包含：
`speech_checked`/`speech_detected`/`speech_ratio`/`speech_seconds` 等。
